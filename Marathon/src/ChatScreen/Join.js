...
class Join extends Component {
  ...
  joinCircle() {
     const { joinKey } = this.state;
        this.setState({loading: true })
        // console.log(this.state.keyval);
        var avalCircle = [];
        let admin = firebase.auth().currentUser.uid;
        firebase.database().ref('Circles/').once('child_added' ,(snap) => {
            let obj = snap.val();
            obj.circleID = snap.key;
            console.log(obj)
            let code;
            for ( var key in obj) {
              if ( joinKey === key.slice(key.length - 5)) {
                code = key;
                firebase.database().ref('Circles/' + code + '/members/').push(admin)
                .then((members) => {
                ToastAndroid.show('You are added', ToastAndroid.SHORT);
              })
            }
          } if (!joinKey === -1) {
                ToastAndroid.show('Please enter your key', ToastAndroid.SHORT);
              }
            // }
            // var circleList = this.state.circleList;
            // circleList.push(obj);
